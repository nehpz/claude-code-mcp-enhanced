...
async function runTest() {
  console.log('Testing task execution with real tasks...');
  
  const taskPath = path.join(process.cwd(), 'docs', 'tasks', 'test_execution_modes.md');
  ...
  const conversionResult = await convertTaskMarkdown({ 
    ...
    outputPath: path.join(process.cwd(), 'test_task.json')
  });
  
  console.log('Task conversion result:', JSON.stringify(conversionResult, null, 2));
  
  if (!conversionResult.success || !conversionResult.task) {
    console.error('Failed to convert task.');
    return;
  }
  
  const taskId = conversionResult.task[0]?.id || 'test_task_' + Date.now();
  console.log(`\nTask ID from conversion: ${taskId}`);
  
  // Step 2: Execute the task (in sequential mode first)
