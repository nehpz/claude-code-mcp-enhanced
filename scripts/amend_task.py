#!/usr/bin/env python3
"""
Amend Task List Command Line Interface

This script provides a command-line interface for amending task markdown files
to conform to the standard template required by the Claude Code MCP.

Usage:
  python amend_task.py input.md -o output.md
  python amend_task.py input.md  # Overwrites the input file
"""
    ...
    try:
        ...
        if not template_path.exists():
            ...
            for path in alt_paths:
                ...
                print("Could not find TASK_LIST_TEMPLATE_GUIDE.md, using default templates")
                return ""
        
        with open(template_path, 'r', encoding='utf-8') as file:
            return file.read()
    except Exception as e:
        print(f"Error loading template guide: {e}")
        return ""
    ...
    try:
        ...
        if output_path:
            with open(output_path, 'w', encoding='utf-8') as file:
                file.write(content)
            print(f"Amended task list saved to {output_path}")
        
        return content
    
    except Exception as e:
        print(f"Error amending task list: {e}")
        raise
...
def main(
    ...
    output: Optional[str] = typer.Option(None, "--output", "-o", help="Path to save the amended file (defaults to overwriting input)"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose output"),
    check: bool = typer.Option(False, "--check", help="Only check for conformance without modifying")
):
    ...
    try:
        if check:
            ...
            if missing_sections:
                ...
                typer.echo("Task file conforms to template.")
                raise typer.Exit(code=0)
        else:
            # Amend the task list
            amended_content = amend_task_list(input_file, output_path)
            
            typer.echo(f"Task list amended successfully and saved to {output_path}")
            typer.echo("You can now convert it to JSON with:")
            typer.echo(f"  python task_to_json.py {output_path}")
