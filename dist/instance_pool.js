...
(function (PoolStrategy) {
    ...
    PoolStrategy["LEAST_RECENTLY_USED"] = "least_recently_used";
    PoolStrategy["LEAST_BUSY"] = "least_busy";
...
export class InstancePool {
    instances = new Map();
    availableInstances = [];
    tasks = new Map(); // taskId -> instanceId
    ...
    cleanupIdleInstances() {
        ...
        if (this.availableInstances.length < this.initialSize && this.instances.size < this.maxInstances) {
            const instancesToCreate = Math.min(this.initialSize - this.availableInstances.length, this.maxInstances - this.instances.size);
     ...
     */
    releaseInstance(taskId, success = true, executionTime = 0) {
     ...
     */
    executeCommand(command, timeout, cwd) {
